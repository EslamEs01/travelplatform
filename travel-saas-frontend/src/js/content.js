...
(function () {
  ...
  document.addEventListener('DOMContentLoaded', function () {
    ...
    else if (page === 'destination-details') { initDestinationDetails(); }
  ...
  function initDestinations() {
    ...
    function getActiveChips() {
      var region = null, style = null;
      document.querySelectorAll('[data-filter="region"][aria-pressed="true"]')
        .forEach(function (b) { region = b.dataset.value; });
      document.querySelectorAll('[data-filter="style"][aria-pressed="true"]')
        .forEach(function (b) { style = b.dataset.value; });
    ...
    function applyFilters() {
      ...
      cards.forEach(function (card) {
        var haystack = normalize(card.dataset.search || '') + ' ' + normalize(card.dataset.destination || '');
        var matchQ = !q || haystack.indexOf(q) !== -1;
        var matchR = !chips.region || normalize(card.dataset.region || '') === normalize(chips.region);
        var matchS = !chips.style  ||
    ...
    function restoreFromUrl() {
      var p = getParams();
      var region = p.get('region'), style = p.get('style'), q = p.get('q');
      ...
      btn.addEventListener('click', function () {
        ...
        if (btn.dataset.value === 'all') {
          ...
          var allBtn = document.querySelector(
            '[data-filter="' + group + '"][data-value="all"]');
    ...
    if (resetBtn) {
      resetBtn.addEventListener('click', function () {
        ...
        allBtns.forEach(function (b) { b.setAttribute('aria-pressed', 'true'); });
    ...
    document.querySelectorAll('[data-save-toggle]').forEach(function (btn) {
      btn.addEventListener('click', function () {
        ...
        if (window.TUI && window.TUI.toast) {
          window.TUI.toast(pressed ? 'تمت إزالة الوجهة من المحفوظات' : 'تمت إضافة الوجهة إلى المحفوظات');
        }
      });
    });
  ...
  function initDestinationDetails() {
    ...
    for (var i = 0; i < catalog.length; i++) {
      if (catalog[i].id === id || catalog[i].slug === id) { dest = catalog[i]; break; }
    }
    if (!dest) {
      // Fallback: find dest-dubai
      for (var j = 0; j < catalog.length; j++) {
        if (catalog[j].id === 'dest-dubai') { dest = catalog[j]; break; }
      }
    }
    ...
    if (dest.name)        { swap('name', dest.name); }
    if (dest.country)     { swap('country', dest.country); }
    if (dest.region)      { swap('region', dest.region); }
    if (dest.description) { swap('description', dest.description); }
    if (dest.priceFrom)   { swap('price', dest.priceFrom.toLocaleString('ar-SA') + ' ' + (dest.currency || 'ر.س')); }
    if (dest.bestSeason)  { swap('season', dest.bestSeason); }
    if (dest.image)       { swapAttr('img', 'src', dest.image); }
    if (dest.imageAlt)    { swapAttr('img', 'alt', dest.imageAlt); }
  ...
  function initBlog() {
    ...
    function applyFilters() {
      var q = normalize(searchEl ? searchEl.value : '');
      var cat = null;
      document.querySelectorAll('[data-filter="category"][aria-pressed="true"]')
        .forEach(function (b) {
    ...
    function restoreFromUrl() {
      var p = getParams();
      var cat = p.get('category'), q = p.get('q');
      if (cat) {
        var cb = document.querySelector(
          '[data-filter="category"][data-value="' + cat + '"]');
    ...
    document.querySelectorAll('[data-filter="category"]').forEach(function (btn) {
      btn.addEventListener('click', function () {
        ...
        if (btn.dataset.value === 'all') {
          document.querySelectorAll('[data-filter="category"][data-value="all"]')
            .forEach(function (b) { b.setAttribute('aria-pressed', 'true'); });
        }
        applyFilters();
      });
    });
  ...
  function initArticle() {
    ...
    if (catalogEl) {
      ...
      if (catalog) {
        ...
        for (var i = 0; i < catalog.length; i++) {
          if (catalog[i].id === id) { art = catalog[i]; break; }
        }
        if (!art) {
          for (var j = 0; j < catalog.length; j++) {
            if (catalog[j].id === 'art-cheap-dubai-deals') { art = catalog[j]; break; }
          }
        }

        if (art) {
          function swap(attr, val) {
            document.querySelectorAll('[data-article-' + attr + ']')
              .forEach(function (el) { el.textContent = val; });
          }
          if (art.title)    { swap('title', art.title); document.title = art.title + ' — رحلاتي'; }
          if (art.excerpt)  { swap('excerpt', art.excerpt); }
          if (art.author)   { swap('author', art.author); }
          if (art.category) { swap('category', art.category); }
          if (art.readingTime) { swap('reading-time', art.readingTime); }
          if (art.relatedDestination) { swap('destination', art.relatedDestination); }
        }
      }
    }
    ...
    document.querySelectorAll('[data-share-btn]').forEach(function (btn) {
      btn.addEventListener('click', function () {
        ...
        if (window.TUI && window.TUI.toast) { window.TUI.toast(msg); }
      });
    });
